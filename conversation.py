...
class ConversationState:
    ...
    LOOKING = "looking"     # Taking a photo (purple eyes)
...
def capture_frame_base64():
    """Capture a JPEG frame from the camera via Flask and return as base64 string.
    Returns (base64_string, mime_type) or (None, None) on failure."""
    try:
        response = requests.get(
            f"{FLASK_URL}/camera/capture",
            timeout=3.0,
            verify=False
        )
        if response.status_code == 200:
            data = response.json()
            return data.get("image"), data.get("mime_type", "image/jpeg")
        else:
            print(f"[CAMERA] Capture failed: HTTP {response.status_code}")
            return None, None
...
TAKE_PHOTO_TOOL = types.Tool(
    function_declarations=[
        types.FunctionDeclaration(
            name="take_photo",
            description=(
                "Capture a photo from CatBot's camera. Use this when the user asks "
                "what you see, what's in front of you, to look at something, or when "
                "visual context would help answer their question."
            ),
    ...
    try:
        ...
        with sd.RawInputStream(
            ...
            async with client.aio.live.connect(model=GEMINI_MODEL, config=config) as session:
                ...
                async def send_audio():
                    ...
                    while conversation_active:
                        try:
                            ...
                            # Don't send mic audio while taking a photo
                            if state == ConversationState.LOOKING:
                                chunks_skipped += 1
                                continue
                ...
                # Task 2: Receive and play Gemini responses (including tool calls)
                ...
                async def monitor_timeout():
                    ...
                    while conversation_active:
                        ...
                        # Don't timeout while Gemini is speaking or looking
                        if state in (ConversationState.SPEAKING, ConversationState.LOOKING):
                            last_activity = time.time()
                            continue
...
async def handle_take_photo(session):
    """Handle the take_photo tool call: flash purple eyes, capture, send image to Gemini."""
    print(" [TOOL] take_photo called")

    # Flash purple eyes
    update_eyes(ConversationState.LOOKING)

    # Capture the frame (run in executor so we don't block the event loop)
        ...
        await session.send_tool_response(
            function_responses=[
                types.FunctionResponse(
                    name="take_photo",
                    response={"error": "Camera capture failed. Describe that you couldn't see anything."}
                )
            ]
        )
    else:
    ...
    update_eyes(ConversationState.LISTENING)
    print(" [TOOL] take_photo complete, returned to listening")
